    ...
    const handleFileChange = (e) => {
        ...
        if (file && file.type === 'application/pdf') {
            if (file.size > 10 * 1024 * 1024) {
                setMessage('PDF must be under 10MB.');
                return;
            }
                ...
                {uploadProgress > 0 && (
                    <div className="mt-4">
                        <div className="h-2 rounded-full bg-gray-200 dark:bg-white/10 overflow-hidden">
                            <div style={{ width: `${uploadProgress}%` }} className="h-full rounded-full bg-gradient-to-r from-brand-500 to-accent-500 transition-all duration-300"></div>
                        </div>
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">Uploading {uploadProgress}%</p>
                    </div>
                )}
